...
| `i2cscan;` | Temporary diagnostic scan of the I2C bus (SDA=8, SCL=9, 100 kHz); prints every detected 7-bit address. |
...
| `rspt[<threshold>[,<ACR>[,<CF>]]],<targetPulses:int>,<targetDose:float>;` | Starts pulse-count mode with GPIO21 target signaling. Defaults for optional fields are `threshold=-9.0`, `ACR=1.0`, `CF=1.0`. Supported forms are `rspt<targetPulses>,<targetDose>;`, `rspt<threshold>,<targetPulses>,<targetDose>;`, `rspt<threshold>,<ACR>,<targetPulses>,<targetDose>;`, and `rspt<threshold>,<ACR>,<CF>,<targetPulses>,<targetDose>;`. `targetPulses` must be an integer greater than 0 and `targetDose` must be a float. Firmware always drives GPIO21 LOW before starting. On each sample it updates pulse count and accumulated dose; when either value reaches/passes its target, GPIO21 is set HIGH. |
| `rsptlast;` | Prints a human-readable summary of the most recent completed `rspt...;` run, including final pulse count and accumulated dose. If no previous `rspt` run has been completed, prints that no result is available. |
